...
def load_experiment_tracking():
    """
    Loads the experiment tracking JSON file if it exists,
...
def model_exists_as_transformer_lens(model_name):
    """
    Checks if a Transformer Lens version of the model exists.
    E.g. checks for `models/tf_lens_modelName`.
    """
    tf_lens_path = os.path.join(MODEL_FOLDER, TRANSFORMER_LENS_PREFIX + model_name)
    return os.path.isdir(tf_lens_path) or os.path.isfile(tf_lens_path)


def create_transformer_lens_model(model_name, verbose=False):
    """
    Calls model_setup.py with --model_name to create the Transformer Lens version
    if it doesn't already exist.
    """
    if verbose:
        print(f"[INFO] Creating Transformer Lens model for {model_name}...")
    cmd = [
        "python",
        "model_setup.py",
        "--model_name",
...
def run_train_probe(model_name, probe_dataset, training_config, verbose=False):
    """
...
def main():
    ...
    for model_name, probe_dataset, training_config in combos:
        ...
        if (model_name, probe_dataset, training_config) in completed_set:
            ...
            # 3) Train the probe
            run_train_probe(model_name, probe_dataset, training_config, verbose=args.verbose)
